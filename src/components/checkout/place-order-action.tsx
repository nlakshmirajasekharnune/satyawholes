...
export const PlaceOrderAction: React.FC<{ className?: string }> = (props) => {
  ...
  const { items } = useCart();
  const { me } = useUser();
  const phoneval=JSON.parse(localStorage.getItem('Phone') || '{}');
 ...
 var payment_gateway=PaymentGateway.COD;
  ...
  let formatRequiredFields = isDigitalCheckout
    ? [phoneval, payment_gateway, available_items]
    : [
      phoneval,
      payment_gateway ,
      billing_address,
