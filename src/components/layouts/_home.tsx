...
export default function HomeLayout({
  children,
  layout,
}: React.PropsWithChildren<{ layout: string ,children:any}>) {
  const { t } = useTranslation('common');
  const [, setDisplayMobileHeaderSearch] = useAtom(
    displayMobileHeaderSearchAtom
  );
  return (   
    <div className="flex flex-col min-h-screen transition-colors bg-gray-100">
      {['minimal', 'compact'].includes(layout) ? (
        <HeaderMinimal layout={layout} />
      ) : (
        <Header layout={layout} variableType={children.props?.variables} />
      )}
      <div className="min-h-screen mt-6">{children}</div>
