...
const AuthorizedMenu: React.FC<{ minimal?: boolean }> = ({ minimal }) => {
  ...
  return (
    <Menu
      as="div"
      className="relative inline-block ltr:text-left rtl:text-right mt-2"
    >
      ...
      >
        <Menu.Items
          as="ul"
          className={cn(
            'absolute mt-3 w-48 rounded bg-white pb-4 shadow-700 focus:outline-none ltr:right-0 ltr:origin-top-right rtl:left-0 rtl:origin-top-left',
            {
              '!mt-3': minimal,
            }
          )}
