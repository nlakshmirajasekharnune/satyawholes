...
const MobileHeader = ({ layout ,variableType}: { layout?: string,variableType:any }) => {
  ...
  return (
    <header
      className={cn(
        'site-header-with-search sticky top-0 z-50 lg:hidden md:hidden',
        {
    ...
    >
      ...
      <div
        className={cn(
          'lg:block md:block xs:hidden  flex h-6 w-full transform-gpu items-center justify-between border-b border-border-200 bg-white px-4 py-5 shadow-sm transition-transform duration-300 md:h-6 lg:h-6 lg:px-6 z-10',
          {
      ...
      >
        ...
        (<ul className="items-center shrink-0 space-x-7 rtl:space-x-reverse lg:flex 2xl:space-x-10">
          <div className="flex items-center space-x-4 rtl:space-x-reverse">
            ...
            <button onClick={()=>(router.push(Routes.wishlists))}>
              <HeartOutlineIcon className="h-5 w-5 text-accent" />
              <p className='uniqueWishlist'>{formatString(wishlistTerm, t(''))}</p>
            </button>
            <button
              className="product-cart lg:flex flex-col items-center pt-2"
      ...
      <ul className="lg:pl-[40px] items-center hidden shrink-0 space-x-7 rtl:space-x-reverse lg:flex 2xl:space-x-10 whitespace-nowrap h-10" >
