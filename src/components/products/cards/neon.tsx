...
const Neon: React.FC<NeonProps> = ({ product, className }) => {
  ...
  const { name, image, quantity, min_price, max_price,ratings, product_type } =
    product ?? {};
  const { price, basePrice, discount } = usePrice({
    amount: product.sale_price ? product.sale_price : product.price!,
    baseAmount: product.price,
  });
  const rtval=Math.round(ratings)
  // console.log(rtval,'gj',ratings);
    ...
    >
      ...
      <header className="p-3 md:p-6">
        ...
        <div className="inline-flex shrink-0 items-center rounded border border-accent bg-accent px-3 py-1 text-sm text-white">
                  ...
                  {/* {Array.from(Array(rtval), (e, i) => { */}
                    {/* return   */}
                    <StarIcon className="h-2.5 w-2.5 ltr:ml-1 rtl:mr-1" />
                    {/* })} */}
                </div>
                
{/* when the rating is not display we can use this below code above code can comment */}
                {/* {
                ratings!=0?(<div className="inline-flex shrink-0 items-center rounded border border-accent bg-accent px-3 py-1 text-sm text-white">
                  {ratings} */}
                  {/* {Array.from(Array(rtval), (e, i) => { */}
                  {/* return   */}
                  {/* <StarIcon className="h-2.5 w-2.5 ltr:ml-1 rtl:mr-1"/> */}
                  {/* })} */}
                  {/* </div>):(<div className="inline-flex shrink-0 items-center px-3 py-1 text-sm text-white"></div>)
                  } */}
