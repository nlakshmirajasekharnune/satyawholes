...
function WishlistItem({ product }: { product: Product }) {
  ...
  const productSingleUrl =
    ...
    if(checkToken!=null)
    {
    vtbc=me?.tobacco_license!=null?(me?.tobacco_license_verify==1?5:(me?.tobacco_license_reason!=null?4:3)):1;
    vbsc=me?.business_license!=null?(me?.business_license_verify==1?5:(me?.business_license_reason!=null?4:3)):1;
    if(vbsc==3&&vtbc!=3)
    {
      vtbc=2;
      vbsc=3;
  ...
  return (
    <div className="flex w-full items-start space-x-4 border-b border-gray-200 py-5 first:pt-0 last:border-0 last:pb-0 rtl:space-x-reverse sm:space-x-5 xl:items-center">
      ...
      <div className="flex w-full flex-col items-start sm:flex-row sm:justify-between sm:space-x-4 rtl:sm:space-x-reverse xl:items-center">
        ...
        <div className="mt-4 flex w-full flex-col justify-between space-y-3 xs:grid xs:space-y-0 sm:w-auto sm:flex-col sm:justify-end sm:space-y-3 md:mt-0">
            ...
            {Number(product?.quantity) > 0 && (
              <>
                {product?.product_type.toLowerCase() === 'variable' ? (
                  ...
                  <div>{(vbsc==5&&(product?.shop?.id =='6'?(vtbc==5):true))?(
                  <AddToCart variant="text" data={product} />):null}
                  </div>
                )}
              </>
            )}
