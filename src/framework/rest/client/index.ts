...
class Client {
  products = {
    all: ({
      ...
      fetchProduct: ({ id }: { id: string }) => {
        return HttpClient.get<Product>(`${API_ENDPOINTS.PRODUCTSEARCH}/${id}`);
      },
  ...
  threeplclient={
    analytics:()=>  HttpClient.get<any>(API_ENDPOINTS.THREEPL)
  ...
  users = {
    ...
    register: (input: RegisterUserInput) =>
      HttpClient.post<any>(API_ENDPOINTS.USERS_REGISTER, input),
    verifyemail: (input: VerifyEmail) =>
      HttpClient.post<any>(API_ENDPOINTS.VERIFYEMAIL, input),
